...
int max1=0,max2=0,max_join,flag4 = 0,flag=0,flag1=0,flag2 = 0,index,flag_zero=0,flag_one = 0,end_right=-1,end_left=-1,second_max_b=-1,second_max_e=-1;
...
void appy(vector<int> arr,int k,string query){
	...
	int n=arr.size(),j,temp_max_b=-1,left_index = -1, right_index = -1, first_max=0,first_max_b = -1,first_max_e = -1, second_max = 0, second_max_b = -1,second_max_e=-1;
...
count = 1,i=0;
	...
	while(i<n){
		...
		if(arr[i] == 1){
			...
			if( (second_max < count) && (temp_max_b != first_max_b) ){			//initialise first and second max so that you can determine if they are 0
				second_max = count;
